#!/usr/bin/env python3
"""
hover_guided_hold_se3.py — Coarse-position-and-hold controller (passive HOLD)

Based on hover_guided_hold.py with the following changes:
  1. Loose HOLD entry / drift tolerances to reduce jitter-induced re-alignment.
  2. hold_loss_grace_frames: survive isolated PnP back-solution frames without exiting HOLD.
  3. HOLD is fully passive (zero velocity) — optical flow holds position during measurement.

STATE MACHINE
-------------
  SEARCH → COARSE_ALIGN → SETTLE → HOLD  (same as hover_guided_hold.py)

Usage (hardware — GUIDED mode):
  ros2 run tag_hover_two_tags hover_guided_hold_se3 --ros-args \\
...
class HoverGuidedHold(Node):
    ...
    def __init__(self):
        super().__init__('hover_guided_hold_se3')
        ...
        # Fix 3 — HOLD loss grace: survive single PnP back-solution frames without exiting HOLD
        self.declare_parameter('hold_loss_grace_frames', 5)    # ticks before HOLD → SEARCH on tag loss
        ...
        # HOLD drift recovery — tighter thresholds (triggers re-alignment before large drift)
        ...
        self.hold_loss_grace_frames = self.get_parameter('hold_loss_grace_frames').get_parameter_value().integer_value
        ...
        self._hold_loss_count   = 0     # Fix 3: ticks since tags lost in HOLD

        period = 1.0 / self.rate_hz if self.rate_hz > 0 else 0.1
        self._timer = self.create_timer(period, self._on_timer)

        self.get_logger().info(
            f"hover_guided_hold_se3 READY | "
            ...
            f"drift tols: lat={self.drift_lat_tol}m dist={self.drift_dist_tol}m | "
            f"hold_loss_grace={self.hold_loss_grace_frames} ticks"
        )
    ...
    def _on_timer(self):
        ...
        # ── HOLD: passive (zero velocity); drift monitor triggers re-alignment ──
        if self._ctrl_state == HOLD:
            ...
            try:
                ...
                self._hold_loss_count = 0  # good read — reset grace counter
                ...
                if self._hold_log_accum >= 5.0:
                    ...
                    self.get_logger().info(
                        f"[HOLD] z0={z0:.2f}m mid_x={mid_x:+.3f}m — passive hold"
                    )

            except TransformException as ex:
                self._hold_loss_count += 1
                if self._hold_loss_count <= self.hold_loss_grace_frames:
                    self.get_logger().warn(
                        f"[HOLD] tag lost — holding still "
                        f"({self._hold_loss_count}/{self.hold_loss_grace_frames} grace) | {ex}",
